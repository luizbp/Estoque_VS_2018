...
namespace Estoque
{
    public partial class PesqGeral : Form
    {
        ...
        private void dgvPesq_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.ColumnIndex == 5 && Convert.ToInt32(e.Value) >= 1)
            {
                dgvPesq.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.LightGreen;
            }
            if (e.ColumnIndex == 5 && Convert.ToInt32(e.Value) <= 0)
            {
                dgvPesq.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.LightCoral;
            }

            if (e.ColumnIndex == 5 && Convert.ToInt32(e.Value) == 0)
            {
                dgvPesq.Rows[e.RowIndex].DefaultCellStyle.SelectionForeColor = Color.White;
            }
        }
